...
async def _notify_manager(tg_client: Client, message: Message, user_name: str, user_text: str, conv_id: int = 0) -> None:
    target = settings.manager_group_id or settings.notification_chat_id
    if not target or not tg_client:
        return
    try:
        now = time.time()
        u = message.from_user
        ...
        if u:
            tg_full = f"{u.first_name or ''} {u.last_name or ''}".strip() or "—"
            tg_at   = f" @{u.username}" if u.username else ""
            user_id = u.id
        else:
            tg_full, tg_at, user_id = "—", "", "?"

        # Reuse existing ticket if same conv_id within 30 min, else create new
        existing = _active_tickets.get(conv_id)
        if existing and (now - existing["ts"]) < _TICKET_REUSE_WINDOW:
            ticket = existing["ticket"]
            is_update = True
            # Delete old ticket message
            try:
                await tg_client.delete_messages(target, existing["msg_id"])
            except Exception as e:
                logger.debug(f"[TICKET] Could not delete old msg: {e}")
        else:
            ticket = _next_ticket_number()
            is_update = False

        # Collect ALL questions this client asked (from full history)
        hist = _history.get(conv_id, [])
        questions = [t["content"] for t in hist if t["role"] == "user"]
        questions_block = "\n".join(
            f"• {_html_escape(q[:150])}" for q in questions[-5:]
        ) if questions else f"• {_html_escape(user_text[:150])}"

        # Last 3 bot answers
        answers = [t["content"] for t in hist if t["role"] == "assistant"]
        last_answer = _html_escape(answers[-1][:200]) if answers else "—"

        tag = "🔄 <b>Заявка обновлена</b>" if is_update else "🔔 <b>Новая заявка</b>"

        # Direct link to the client's last message in the group
        msg_link = _tg_link(message.chat.id, message.id)
        link_line = f'\n💬 <a href="{msg_link}">Открыть переписку в группе →</a>' if msg_link else ""

        text = (
            f"{tag} <b>#{ticket:03d}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━\n\n"
            f"👤 <b>{_html_escape(tg_full)}</b>{_html_escape(tg_at)}\n"
            f"🆔 <code>{user_id}</code>  |  💬 {_html_escape(chat_title)}"
            f"{link_line}\n\n"
            f"❓ <b>Все вопросы клиента:</b>\n{questions_block}\n\n"
            f"🤖 <b>Последний ответ бота:</b>\n{last_answer}\n\n"
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"✍️ Кто берёт — пишите: <b>Беру #{ticket:03d}</b>"
        )
        sent = await tg_client.send_message(target, text, parse_mode=enums.ParseMode.HTML)
        _active_tickets[conv_id] = {"ticket": ticket, "msg_id": sent.id, "ts": now}
        action = "updated" if is_update else "created"
        logger.info(f"[AI] Ticket #{ticket:03d} {action} for conv={conv_id}")
    except Exception as e:
        logger.error(f"[AI] Could not notify manager: {e}")
